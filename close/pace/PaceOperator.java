...
public class PaceOperator {
	...
	private byte[] calcAuthToken(byte[] kmac, byte[] data) {
		logger.debug("Calculating auth token for data: " + HexString.bufferToHex(data));
		logService.logDebug("Calculating auth token for data: " + HexString.bufferToHex(data));

		byte[] tpcd = null;

		try {
			// Reinitialize crypto with MAC key and zero IV
			byte[] zeroIV = new byte[crypto.getBlockSize()];
			crypto.init(kmac, zeroIV);

			if (pace instanceof PaceECDH) {
				ECCurve.Fp curve = (ECCurve.Fp) dp.getECParameter().getCurve();
				...
				// Get encoded form for MAC calculation
				byte[] encodedKey = pkpcd.getEncoded();
				logger.debug("Encoded key for auth token: " + HexString.bufferToHex(encodedKey));
				logService.logDebug("Encoded key for auth token: " + HexString.bufferToHex(encodedKey));
			...
			logService.logDebug("Calculated auth token: " + HexString.bufferToHex(tpcd));
			return tpcd;
		} catch (Exception e) {
			logger.error("Error calculating auth token", e);
			logService.logError("Error calculating auth token", e);
			return null;
		}
	}
...
// Important: Add these changes to PaceOperator class
	...
	private DynamicAuthenticationData performMutualAuthentication(byte[] authToken) throws SecureMessagingException, CardException, PaceException {
		...
		// Try using a more direct encoding approach
		// Format: [7C][Len][85][Len][auth token]
		byte[] data = new byte[authToken.length + 4];
		data[0] = (byte)0x7C;  // Dynamic Authentication tag
		data[1] = (byte)(authToken.length + 2);  // Length of remaining data
		data[2] = (byte)0x85;  // Tag 5 for authentication token
		...
		logger.debug("Raw mutual auth data: " + HexString.bufferToHex(data));
		logService.logDebug("Raw mutual auth data: " + HexString.bufferToHex(data));

		// Try various APDU formats
		CommandAPDU capdu;

		// Option 1: Standard format with Le=0x00
		capdu = new CommandAPDU(0x00, 0x86, 0x00, 0x00, data, 0x00);
		logger.debug("Mutual Auth APDU (Le=0x00): " + HexString.bufferToHex(capdu.getBytes()));

		ResponseAPDU resp;
		try {
			resp = cardHandler.transfer(capdu);
			logger.debug("Mutual Auth Response: " + HexString.bufferToHex(resp.getBytes()));
			logService.logDebug("Mutual Auth Response: " + HexString.bufferToHex(resp.getBytes()));

			if (resp.getSW() != 0x9000 && resp.getSW() != 0x6282) {
				// Try option 2: No Le field
				logger.debug("Trying without Le field...");
				capdu = new CommandAPDU(0x00, 0x86, 0x00, 0x00, data);
				logger.debug("Mutual Auth APDU (no Le): " + HexString.bufferToHex(capdu.getBytes()));
				...
				if (resp.getSW() != 0x9000 && resp.getSW() != 0x6282) {
					// Try option 3: Extended length with 256 as Le
					logger.debug("Trying with extended length...");
					capdu = new CommandAPDU(0x00, 0x86, 0x00, 0x00, data, 256);
					logger.debug("Mutual Auth APDU (Le=256): " + HexString.bufferToHex(capdu.getBytes()));
			...
			logger.error("Error in mutual authentication", e);
			logService.logError("Error in mutual authentication", e);
			throw new PaceException("Error in mutual authentication", e);
		}
	}
